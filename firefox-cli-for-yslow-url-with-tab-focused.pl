...
if ( $numArgs ne 0 ) {
	...
	print "\nLast Tab Obj= $LastTab";
	print "\nLast- index=> $LastTab->{'index'}";
	print "\nLast- title=> $LastTab->{'title'}";
	print "\nLast- document=> $LastTab->{'document'}";
	print "\nLast- tab=> $LastTab->{'tab'}";
	print "\nLast- panel=> $LastTab->{'panel'}";
	my $LastTabRef = $LastTab->{'tab'};
	$ffobj->activateTab($LastTabRef);
	#my $tabobj = $ffobj->addTab(autoclose => 0); 
	#while (my ($key,$value)=  each(%{$LastTab})){  print "\n $key => $value"; }
	foreach my $key (sort keys %{$LastTab}) {
		print "\nKey- $key";
	}
